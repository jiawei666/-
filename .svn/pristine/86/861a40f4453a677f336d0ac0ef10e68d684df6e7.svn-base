 ...
 * 语言检测 并自动加载语言包
 */
class OnemlaLangBehavior extends Behavior{
    ...
    private function checkLanguage() {
        ...
        // 读取模块语言包
        $file   =   APP_PATH.'Lang/'.LANG_SET.'.php';
        if(is_file($file))
            L(include $file);

        // 读取当前控制器语言包
        $file   =  LANG_PATH.LANG_SET.'/'.strtolower(COMPONENTS_INPUT).'.php';
        if (is_file($file))
            L(include $file);

        // 站内消息
        $file   =  LANG_PATH.LANG_SET.'/Onemla.Message.php';
        if (is_file($file))
            L(include $file);
    }
}
