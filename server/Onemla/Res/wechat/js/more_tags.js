...
$(function(){
    ...
    $('div[name="more"]').click(function () {
        ...
        var tags = $('div[class="home-subnav"]').find('a[class="active"]').attr('name');
        var bsn = $('input[name="bsn_id"]').val();
        if(tags!='' && bsn!='') {
            var where = "`r_bsn_id`="+bsn+" and `lock`=1 and `audit_status`=2";
        }else {
            alert('请求失败');
            return;
        }
        $.post(url,{number:number,where:where,tags:tags},function(data){
