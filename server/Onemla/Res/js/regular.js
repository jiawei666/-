    ...
    {
        if(!$('input[name="register-user"]').val()==''){
            if($.trim($('input[name="register-user"]').val()).length==0){
                alert('用户名不能为空');
                return false;
            }
        }else {
            $('input[name="register-user"]').attr('required','required')
        }
        ...
        {//邮箱
            ...
            $('input[name="register-email"]').attr('required','true');
        }

        if(!$('input[name="register-tel"]').val()=='')
        {//手机号码
            function isPhone(phone) {
                var pattern = /^1[34578]\d{9}$/;
                return pattern.test(phone);
            }
            var phone=$('input[name="register-tel"]').val();
            if(!isPhone(phone)){
                alert('请输入正确的手机号');
                return false;
            }
        }else {
            $('input[name="register-tel"]').attr('required','true');
        }
