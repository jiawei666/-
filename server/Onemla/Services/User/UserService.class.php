...
class UserService extends OnemlaService {
    ...
    public function registerUser($data) {
        ...
        $audit_submit_times_model = M('audit_submit_times');
        $pInsert = array(
        ...
        if ($uid > 0) {
            ...
            if($id > 0){//注册同时添加会员认证信息;
                $audit_submit_InsertInfo = array(
                    'user_id' => $uid,
                    'reset_time' => date("Y-m-d H:i:s"),
                );
                $audit_submit_times_model->add($audit_submit_InsertInfo);
//                if ($audit_Insertres == 0) {
//                    $audit_submit_times_model->delete(array(
//                        'id' => $uid
//                    ));
//                    return false;
//                }
...
//        }
        ...
        $pUser = new User();
        $pUser->setUserInfo($pUserInfo);//将用户信息打包成数组然后储存在session
        $pUser->updateSession();
